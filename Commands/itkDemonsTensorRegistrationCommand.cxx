...
struct DemonsTensorRegistrationCommandArguments
{
  ...
  friend std::ostream&
  operator<<(std::ostream& o, const DemonsTensorRegistrationCommandArguments& args)
  {
...
template < unsigned int VImageDimension = 3, class SolverPrecision = float, class TensorRealType = float, class VectorRealType = float >
  class DemonsTensorRegistrationCommandIterationUpdate : public itk::Command
  {
  public:
    typedef DemonsTensorRegistrationCommandIterationUpdate Self;
...
typedef    typename WarpGradientCalculatorType::OutputType WarpGradientType;
  ...
  protected:
    DemonsTensorRegistrationCommandIterationUpdate() :
...
void
DemonsTensorRegistrationCommand_help_callback()
{
  std::cout << std::endl;
...
template < unsigned int Dimension, class SolverPrecision, class TensorRealType, class VectorRealType >
  void
  DemonsRegistrationTensorFunction(DemonsTensorRegistrationCommandArguments args)
  {
    ...
    typename DeformationFieldType::Pointer defField = 0;
      ...
      {//for mem allocations
        ...
        if ( args.verbosity > 0 )
          {
            ...
            typename DemonsTensorRegistrationCommandIterationUpdate<Dimension, SolverPrecision, TensorRealType, VectorRealType>::Pointer observer =
            DemonsTensorRegistrationCommandIterationUpdate<Dimension, SolverPrecision, TensorRealType, VectorRealType>::New();
        ...
        if ( args.verbosity > 0 )
          {
            // Create the Command observer and register it with the registration filter.
            typename DemonsTensorRegistrationCommandIterationUpdate<Dimension, SolverPrecision, TensorRealType, VectorRealType>::Pointer multiresobserver =
            DemonsTensorRegistrationCommandIterationUpdate<Dimension, SolverPrecision, TensorRealType, VectorRealType>::New();
...
namespace itk
{
  ...
  DemonsTensorRegistrationCommand::DemonsTensorRegistrationCommand()
  {
    m_ShortDescription = "Non-linear diffeomorphic tensor registration using the demons";
  ...
  int DemonsTensorRegistrationCommand::atoi_check(const char * str)
  {
  char *endptr;
  ...
  std::vector<unsigned int> DemonsTensorRegistrationCommand::parseUIntVector(const std::string & str)
  {
  ...
  void DemonsTensorRegistrationCommand::
  parseOpts(int argc, const char **argv, struct DemonsTensorRegistrationCommandArguments & args)
  {
  // Command line parser
  MetaCommand command;
  command.SetParseFailureOnUnrecognizedOption(true);
  command.SetHelpCallBack(DemonsTensorRegistrationCommand_help_callback);
  ...
  int DemonsTensorRegistrationCommand::Execute(int argc, const char *argv[])
  {
    struct DemonsTensorRegistrationCommandArguments args;
