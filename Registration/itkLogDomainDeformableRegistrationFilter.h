...
public:
	...
	itkTypeMacro( LogDomainDeformableRegistrationFilter, DenseFiniteDifferenceImageFilter );
	...
	/** Set/Get minimum number of iterations to perform */
	itkSetMacro( MinimumNumberOfIterations, int );
	itkGetConstMacro( MinimumNumberOfIterations, int );

	/* Enable/Disable automatic stopping when MSE stops decreasing */
	itkSetMacro( HaltWhenMSEStopsDecreasingFlag, bool );
	itkGetConstMacro( HaltWhenMSEStopsDecreasingFlag, bool );
	itkBooleanMacro( HaltWhenMSEStopsDecreasingFlag );

	/** Set/Get minimum MSE to reach */
	itkSetMacro( MinimumMetric, int );
	itkGetConstMacro( MinimumMetric, int );

	/** Set/Get stop registration tolerance. Registration is stopped if
	 * |MSE(n) - MSE(n-1) < StopRegistrationTolerance * |MSE(n)| */
	itkSetMacro( StopRegistrationTolerance, double );
	itkGetConstMacro( StopRegistrationTolerance, double );
...
private:
	...
	/** Variables related to the automatic stopping criteria */
	bool   m_HaltWhenMSEStopsDecreasingFlag;
	int    m_MinimumNumberOfIterations;
	double m_PreviousMetric;
	double m_MinimumMetric;
	double m_StopRegistrationTolerance;
};
