...
::LogDomainDeformableRegistrationFilter()
{
	...
	m_HaltWhenMSEStopsDecreasingFlag = false;
	m_MinimumNumberOfIterations = -1;
	m_MinimumMetric = 0.0;
	m_PreviousMetric = DBL_MAX;
	m_StopRegistrationTolerance = 1.0e-4;
}
...
::Halt()
{
	// Registration stopped from the outside
	if ( m_StopRegistrationFlag )
	{
		return true;
	}

	// Check whether we have performed the minimum number of iterations (we should check the maximum number here too)
	if ( m_MinimumNumberOfIterations > 0 )
	{
		if ( this->GetElapsedIterations() < (static_cast< unsigned int >( m_MinimumNumberOfIterations )) )
			return false;
	}

	// Automatic stopping requires at least one iteration (do we need this as m_PreviousMetric should be DBL_MAX?
	if ( this->GetElapsedIterations() > 1 )
	{
		double currentMetric = this->GetMetric();
		if ( m_HaltWhenMSEStopsDecreasingFlag )
		{
			// Stop registration if MSE increases or stops decreasing significantly
			if ( m_PreviousMetric - currentMetric < m_StopRegistrationTolerance * currentMetric )
				return true;

			m_PreviousMetric = currentMetric;
		}

		// Stop registration when MSE reaches minimum metric
		if ( currentMetric <= m_MinimumMetric )
			return true;
	}
