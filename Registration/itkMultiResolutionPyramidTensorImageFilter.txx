/*=========================================================================
 ...
 //Edited by Thomas Yeo, CSAIL, MIT @ Asclepios, INRIA
 edited itkMultiResolutionPyramidImageFilter.txx


 =========================================================================*/
#ifndef _itkMultiResolutionPyramidTensorImageFilter_txx
#define _itkMultiResolutionPyramidTensorImageFilter_txx
...
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::MultiResolutionPyramidTensorImageFilter()
{
  m_NumberOfLevels = 0;
...
template < class TInputImage, class TOutputImage >
void
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::SetNumberOfLevels(
    unsigned int num)
{
  ...
  ScheduleType
  temp(m_NumberOfLevels, ImageDimension);
  temp.Fill(0);
  m_Schedule = temp;
...
template < class TInputImage, class TOutputImage >
void
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::SetStartingShrinkFactors(
    unsigned int factor)
{
...
template < class TInputImage, class TOutputImage >
const unsigned int *
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::GetStartingShrinkFactors() const
{
...
template < class TInputImage, class TOutputImage >
void
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::SetSchedule(
    const ScheduleType& schedule)
{

  if ( schedule.rows() != m_NumberOfLevels || schedule.columns()
      != ImageDimension )
    {
...
template < class TInputImage, class TOutputImage >
bool
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>::IsScheduleDownwardDivisible(
    const ScheduleType& schedule)
{
...
template <class TInputImage, class TOutputImage>
void
MultiResolutionPyramidTensorImageFilter<TInputImage, TOutputImage>
::GenerateData()
{
  ...
  typename TOutputImage::Pointer smoothedOutput = TOutputImage::New();
      ...
      for( idim = 0; idim < ImageDimension; idim++ )
        {
          ...
          sigma[idim] = 0.5 * static_cast<float>( factors[idim] ) * this->GetInput()->GetSpacing()[idim];
        }

      typename SmootherType::Pointer smoother0 = SmootherType::New();
      typename SmootherType::Pointer smoother1 = SmootherType::New();
      typename SmootherType::Pointer smoother2 = SmootherType::New();

      smoother0->SetInput( caster->GetOutput() ); smoother0->SetDirection(0);
      smoother1->SetInput( smoother0->GetOutput() ); smoother1->SetDirection(1);
      smoother2->SetInput( smoother1->GetOutput() ); smoother2->SetDirection(2);
      
      smoother0->SetSigma(sigma[0]);
      smoother1->SetSigma(sigma[1]);
      smoother2->SetSigma(sigma[2]);

      smoother2->Update();
      smoothedOutput = smoother2->GetOutput();

      // if the original input was zero, then make sure it still is (for memory reasons)
      typedef ImageRegionConstIterator<TInputImage> ConstIteratorType;
      ConstIteratorType inputIt = ConstIteratorType(this->GetInput(),
          this->GetInput()->GetLargestPossibleRegion());

      typedef ImageRegionIterator<TInputImage> IteratorType;
      IteratorType outputIt = IteratorType(smoothedOutput,
          smoothedOutput->GetLargestPossibleRegion());

      for(inputIt.GoToBegin(); !inputIt.IsAtEnd(); ++inputIt, ++outputIt)
        {
          if(inputIt.Get() == NumericTraits<typename TInputImage::PixelType>::Zero)
            {
              outputIt.Set(NumericTraits<typename TOutputImage::PixelType>::Zero);
            }
        }

      // use mini-pipeline to compute output
      shrinker->SetInput(smoothedOutput);
